...
if(device_id===""){
 ...
 const db = auditDb();
 const newDb=useDB()
 const [constent]= await newDb.query(
  'select * from device_config_audit'
 );
    const [result] = await db.query(
      'select * from devices_audit_report where device_id=?',
      [device_id]
    );
  ...
  const auditTableRows = data.auditData.map(row => {
  ...
  <tr>
          <td>${row.sr_no??'N/A'}</td>
          <td class="date-time">${`${formattedDate} <br>${formattedTime}`??'N/A'}</td>
...
const pdfBuffer = await page.pdf({
  ...
  event.node.res.setHeader('Content-Disposition', 'inline; filename="device-report.pdf"');
