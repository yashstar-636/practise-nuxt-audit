<template>
  <div class="p-6">
    <div class="flex justify-between items-center mb-6">
      <h2 class="text-2xl font-bold text-blue-700">Device Configuration Audit Logs</h2>
      ...
      <table class="min-w-full border border-gray-200">
        ...
        <tbody>
          <tr v-for="log in auditLogs" :key="log.audit_id" class="hover:bg-gray-100">
            <td class="px-4 py-2 border text-center">{{ log.audit_id }}</td>
            <td class="px-4 py-2 border text-center">{{ log.device_id }}</td>
            <td class="px-4 py-2 border text-center text-green-600">{{ log.new_device_name }}</td>
            <td class="px-4 py-2 border text-center text-green-600">{{ log.new_device_type }}</td>
            <td class="px-4 py-2 border text-center text-green-600">{{ log.new_min_temp }}</td>
            <td class="px-4 py-2 border text-center text-green-600">{{ log.new_max_temp }}</td>
...
onMounted(async () => {
  try {
  const result=await $fetch('/api/devices_audit')
