  ...
  <div class="p-6">
    ...
    <form @submit.prevent="updateDevice" class="grid grid-cols-3 gap-4 bg-white p-6 rounded shadow">
      ...
      <div class="col-span-3 flex justify-end gap-3 mt-4">
        ...
        <button type="reset" class="bg-gray-200 text-gray-700 px-4 py-2 rounded">
          Clear All
        </button>
      </div>
    </form>
...
function updateDevice(){
  ...
  device.sendingInterval===""){
    ...
    addUser()
    addLogs({device_id:device.deviceId,module:'Device Config',
    action:"Updated",user_id:"rathodyash636@gmail.com",
    description:`${device.deviceId} configration updated`
})
  }
}
...
async function addLogs(datas) {
  const result=await $fetch('/api/audit_log', {
    method: 'POST',
    body: { data:datas},
  })
  console.log(result)
